#!/usr/bin/env python3
"""
ShenSi-CFD V5 Training (DDP Version)

Architecture: pure regression with physics-informed decoder
- Shared Swin-UNet encoder produces condition features
- PhysicsInformedVerticalDecoder: 4 independent heads
  - u/v: shallow 2-layer heads
...
Loss: MSE + L1 for all variables, optional w/k weighting

Usage:
  NCCL_P2P_DISABLE=1 torchrun --nproc_per_node=4 --master_port=29502 train_v5_ddp.py
  NCCL_P2P_DISABLE=1 torchrun --nproc_per_node=4 --master_port=29502 train_v5_ddp.py --resume
"""
...
from src.models.swin_unet_v5 import create_v5_model
...
def parse_args():
    parser = argparse.ArgumentParser(description='ShenSi-CFD V5 DDP Training')
    ...
    parser.add_argument('--mse-weight', type=float, default=1.0, help='Overall MSE loss weight')
    parser.add_argument('--l1-weight', type=float, default=0.3, help='Overall L1 loss weight')
    parser.add_argument('--w-weight', type=float, default=2.0, help='Additional w loss weight (on top of base)')
    parser.add_argument('--k-weight', type=float, default=2.0, help='Additional k loss weight (on top of base)')
...
def train():
    ...
    ckpt_dir = Path(args.ckpt_dir) if args.ckpt_dir else Path('checkpoints/shensiv5_main')
    log_dir = Path(args.log_dir) if args.log_dir else Path('logs/shensiv5_main')
    if is_main:
    ...
    if is_main:
        print('\n' + '='*70)
        print('ShenSi-CFD V5 Training (DDP)')
        print('  Pure regression + physics-informed decoder')
        print('  w: terrain prior (u·dz/dx + v·dz/dy)')
        print('  k: turbulence prior (C_μ × |∇u|² + |∇v|²)')
        print('='*70)
        print(f'   Local Rank: {local_rank}')
        print(f'   Batch Size: {batch_size}/GPU, Effective: {batch_size * accum_steps * dist.get_world_size()}')
        print(f'   Epochs: {epochs}')
        print(f'   Loss weights: mse={args.mse_weight}, l1={args.l1_weight}')
        print(f'                 w_extra={args.w_weight}, k_extra={args.k_weight}')
    ...
    resume_path = None
    if args.resume:
        resume_path = ckpt_dir / 'latest_v5.pt'
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # V5 forward: pure regression with physics-informed decoder
            outputs = model(inputs)  # (B, 27, 4, H, W)

            # Loss computation: per-variable weighted MSE + L1
            # u/v: base weight, w/k: boosted weight to strengthen gradient signal
            u_mse = F.mse_loss(outputs[:, :, 0], targets[:, :, 0])
            v_mse = F.mse_loss(outputs[:, :, 1], targets[:, :, 1])
            w_mse = F.mse_loss(outputs[:, :, 2], targets[:, :, 2])
            k_mse = F.mse_loss(outputs[:, :, 3], targets[:, :, 3])

            u_l1 = F.l1_loss(outputs[:, :, 0], targets[:, :, 0])
            v_l1 = F.l1_loss(outputs[:, :, 1], targets[:, :, 1])
            w_l1 = F.l1_loss(outputs[:, :, 2], targets[:, :, 2])
            k_l1 = F.l1_loss(outputs[:, :, 3], targets[:, :, 3])

            loss = (u_mse + v_mse + w_mse * w_extra_w + k_mse * k_extra_w) * mse_w \
                 + (u_l1 + v_l1 + w_l1 * w_extra_w + k_l1 * k_extra_w) * l1_w
            ...
            train_losses_dict['total_mse'] = train_losses_dict.get('total_mse', 0) + (u_mse + v_mse + w_mse * w_extra_w + k_mse * k_extra_w).item()
            train_losses_dict['total_l1'] = train_losses_dict.get('total_l1', 0) + (u_l1 + v_l1 + w_l1 * w_extra_w + k_l1 * k_extra_w).item()
            train_losses_dict['u_mse'] = train_losses_dict.get('u_mse', 0) + u_mse.item()
            train_losses_dict['v_mse'] = train_losses_dict.get('v_mse', 0) + v_mse.item()
            train_losses_dict['w_mse'] = train_losses_dict.get('w_mse', 0) + w_mse.item()
            train_losses_dict['k_mse'] = train_losses_dict.get('k_mse', 0) + k_mse.item()
            n_batches += 1

            if is_main and (batch_idx + 1) % 100 == 0:
                print(f'  [{batch_idx+1}/{len(train_loader)}] Loss: {loss.item():.4f} '
                      f'(u_mse={u_mse.item():.4f}, w_mse={w_mse.item():.4f}, k_mse={k_mse.item():.4f})')
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # Validation loss (same formula as training)
                u_mse = F.mse_loss(outputs[:, :, 0], targets[:, :, 0])
                v_mse = F.mse_loss(outputs[:, :, 1], targets[:, :, 1])
                w_mse = F.mse_loss(outputs[:, :, 2], targets[:, :, 2])
                k_mse = F.mse_loss(outputs[:, :, 3], targets[:, :, 3])
                ...
                w_l1 = F.l1_loss(outputs[:, :, 2], targets[:, :, 2])
                k_l1 = F.l1_loss(outputs[:, :, 3], targets[:, :, 3])
                val_loss_batch = (u_mse + v_mse + w_mse * w_extra_w + k_mse * k_extra_w) * mse_w \
                               + (u_l1 + v_l1 + w_l1 * w_extra_w + k_l1 * k_extra_w) * l1_w
        ...
        if is_main:
            ...
            if (epoch + 1) % 10 == 0:
                torch.save({
                    ...
                    'input_mean': train_dataset.stats['input_mean'],
                    'input_std': train_dataset.stats['input_std'],
                }, ckpt_dir / f'shensiv5_main_epoch{epoch+1}.pt')
