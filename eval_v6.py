#!/usr/bin/env python3
"""Evaluate shensiv6 (SwinUNetV6, enhanced decoder + vertical smoother) on test set.

Usage:
    python eval_v6.py
    python eval_v6.py --ckpt path/to/ckpt.pt
"""
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate shensiv6 on test set')
    parser.add_argument('--ckpt', type=str, default='checkpoints/shensiv6_main/best_model_v6.pt',
    ...
    if hasattr(test_dataset, 'stats') and test_dataset.stats is not None:
        ...
        print(f'  Output norm: mean={output_mean.shape}, std={output_std.shape}')
    ...
    print(f'  Parameters: total={params["total"]:,}, decoder={params["decoder"]:,}')
    print(f'  C_μ = {model.vertical_decoder.C_mu.item():.4f}')
    print(f'  VS_α = {model.vertical_decoder.vertical_smoother.alpha.item():.4f}')
    ...
    print(f'\n{"="*60}')
    print('shensiv6 (Enhanced Decoder + VerticalSmoother) Test Set Results')
    print(f'{"="*60}')
    ...
    # Per-level average R²
    print(f'\nPer-level average R² (mean across 27 levels):')
    for c, name in enumerate(var_names):
        level_r2s = []
        for lvl in range(L):
            level_r2s.append(r2(pred[:, lvl, c], target[:, lvl, c]))
        avg_r2 = np.mean(level_r2s)
        print(f'  {name}: {avg_r2:.4f}')
