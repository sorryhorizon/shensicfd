#!/usr/bin/env python3
"""Evaluate shensiv2 (baseline SwinUNetLite) on test set.

Usage:
    python eval_v2.py
    python eval_v2.py --ckpt path/to/ckpt.pt
"""
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate shensiv2 baseline on test set')
    parser.add_argument('--ckpt', type=str, default='checkpoints/shensiv2_baseline/best_model.pt',
    ...
    # v2 baseline uses old 4-channel model (no slope_encoder)
    # Current code has 6-channel model with slope_encoder
    # Load with strict=False and handle mismatches
    model = create_lite_model(config={
        'in_channels': 6, 'base_channels': 32, 'bottleneck_depth': 4, 'window_size': (5, 5),
    ...
    # Load matching keys only (skip slope_encoder which doesn't exist in v2 ckpt)
    ...
    if skipped_keys:
        print(f'Skipped: {skipped_keys[:5]}...')
    model.eval()
    ...
    print(f'\n{"="*60}')
    print('shensiv2 (Baseline SwinUNetLite) vs FuXi-CFD (Test Set)')
    print(f'{"="*60}')
